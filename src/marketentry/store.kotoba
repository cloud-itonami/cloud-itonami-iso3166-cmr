(ns marketentry.store
  "SSoT for the CMR market-entry compliance actor, behind a `Store`
  ...
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Systems CM" :portal "ARMP e-procurement"
    ...
    "eng-4" {:id "eng-4" :operator "Higashi Export" :portal "ARMP e-procurement"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-cm-entity? true :has-cm-entity? false
             :requires-rccm? true :rccm-verified? true
             :drafted? false :submitted? false
             :jurisdiction "CMR" :status :intake}
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               requires-cm-entity? has-cm-entity?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-cm-entity?)  (assoc :engagement/requires-cm-entity? requires-cm-entity?)
    (some? has-cm-entity?)       (assoc :engagement/has-cm-entity? has-cm-entity?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/requires-cm-entity? :engagement/has-cm-entity?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-cm-entity? (boolean (:engagement/requires-cm-entity? m))
     :has-cm-entity? (boolean (:engagement/has-cm-entity? m))
...
(defn kotobase-store
  ...
  [{:keys [db-api seq-fn]}]
  (->KotobaseStore (persist/store {:db-api db-api :actor "marketentry-cmr"})
