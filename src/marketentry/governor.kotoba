(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a Cameroon-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Cameroon-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-cm-entity?
                                       true` (near-universal for Cameroon
                                       ...
                                       Grounded in Cameroonian legal entity
...
(defn- cm-entity-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-cm-entity? true`, INDEPENDENTLY verify
  `:has-cm-entity?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-cm-entity?` ground truth (most Cameroon public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-cm-entity? e))
                 (not (true? (:has-cm-entity? e))))
        [{:rule :cm-entity-missing
          :detail (str subject " はカメルーン居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (cm-entity-missing-violations request st)
